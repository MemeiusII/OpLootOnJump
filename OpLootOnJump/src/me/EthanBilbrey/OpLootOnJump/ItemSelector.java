...
public class ItemSelector 
{
	public static ItemStack getRandItem() 
	{
		...
		ItemStack creeperegg = new ItemStack(Material.CREEPER_SPAWN_EGG);
		ItemStack blazeegg = new ItemStack(Material.BLAZE_SPAWN_EGG);
		ItemStack enderegg = new ItemStack(Material.ENDERMAN_SPAWN_EGG);
		...
		switch (itemNumber) {
			case 1 :
				armorPiece = (int) (Math.random() * (armor.size() - 1));
				itemSelected = armor.get(armorPiece);
				break;
			case 2 :
				int tool = (int) (Math.random() * (tools.size() - 1));
				itemSelected = tools.get(tool);
				break;
			case 3 :
				int item = (int) (Math.random() * (items.size() - 1));
